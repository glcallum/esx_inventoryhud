...
function refreshPropertyMotelInventory()
    ESX.TriggerServerCallback(
        "nextgen-motels:getPropertyInventory",
        function(inventory)
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            TriggerServerEvent("nextgen-motels:putItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, count)
        end
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            TriggerServerEvent("nextgen-motels:getItem", ESX.GetPlayerData().identifier, data.item.type, data.item.name, tonumber(data.number))
        end
