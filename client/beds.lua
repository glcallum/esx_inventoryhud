...
function refreshPropertyMotelBedInventory()
    ESX.TriggerServerCallback(
        "nextgen-motels:getPropertyInventoryBed",
        function(inventory)
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            TriggerServerEvent("nextgen-motels:putItemBed", ESX.GetPlayerData().identifier, data.item.type, data.item.name, count)
        end
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            TriggerServerEvent("nextgen-motels:getItemBed", ESX.GetPlayerData().identifier, data.item.type, data.item.name, tonumber(data.number))
        end
